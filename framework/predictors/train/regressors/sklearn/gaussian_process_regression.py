...
class GaussianProcessRegression(Predictor):
    """
    This class implements a gaussian process regression model from the SKlearn library.
    """
    ...
    def get_name(self) -> str:
        ...
        """
        name = f"{self.library} Gaussian Process Regression"
        return name

    def build_model(self, filtered_model_params: Dict) -> GaussianProcessRegressor:
        """
        This function initializes the Gaussian Process Regression model with the
        model params.
        :return model: The built model
        """
        model = GaussianProcessRegressor(
            kernel=filtered_model_params.get('kernal', None),
            alpha=filtered_model_params.get('alpha', None),
            optimizer=filtered_model_params.get('optimizer', None),
            n_restarts_optimizer=filtered_model_params.get('n_restarts_optimizer', None),
            normalize_y=filtered_model_params.get('normalize_y', None),
            copy_X_train=filtered_model_params.get('copy_X_train', None),
            random_state=filtered_model_params.get('random_state', None)
        )
        return model

    def train_model(self, model: GaussianProcessRegressor,
                    data_x_train: pd.DataFrame, data_y_train: pd.DataFrame,
                    data_x_val: pd.DataFrame, data_y_val: pd.DataFrame,
                    data_x_test: pd.DataFrame, data_y_test: pd.DataFrame) -> [GaussianProcessRegressor,
                                                                              Dict]:
        """
    ...
    def serialize_model(self, model: GaussianProcessRegressor):
        """
        ...
        onnx_model = onnxmltools.convert_sklearn(model=model,
                                                 ...
                                                 initial_types=[('float_input', FloatTensorType(
                                                     [None, len(self.data_x.columns.values)]))])
